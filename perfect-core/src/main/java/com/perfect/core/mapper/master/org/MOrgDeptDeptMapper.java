...
public interface MOrgDeptDeptMapper extends BaseMapper<MOrgDeptDeptEntity> {
    ...
    @Select("    "
        ...
        + "         and t1.tenant_id = #{p2}                                    "
        + "                                                                     ")
    MOrgDeptDeptEntity getODDEntityByCurrentId(@Param("p1") Long current_id, @Param("p2") Long tenant_id);
    ...
    @Select("    "
        ...
        + "                                                                     ")
    int getODDRelationCount(@Param("p1") MOrgEntity searchCondition);

    @Delete("                                                                        "
        + "     delete from m_org_Dept_Dept t                                        "
        + "      where t.current_id = #{p1}                                          "
    )
    int delODDRelation(@Param("p1")Long id);
