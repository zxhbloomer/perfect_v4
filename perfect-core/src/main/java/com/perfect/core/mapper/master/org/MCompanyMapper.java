...
public interface MCompanyMapper extends BaseMapper<MCompanyEntity> {
    ...
    @Select("    "
        ...
        + "         LEFT JOIN v_dict_info AS t3 ON t3.code = '" + PerfectDictConstant.DICT_SYS_COMPANY_TYPE + "' and t3.dict_value = t1.type    "
        + "  where true                                                                                               "
        ...
        + "                     select 1                                                                              "
        + "                       from m_org subt1                                                                    "
        + "                      where subt1.serial_type = '"+ PerfectDictConstant.DICT_SYS_CODE_TYPE_M_COMPANY +"'   "
    ...
    @Select("    "
        ...
        + "         LEFT JOIN m_staff c_staff ON t1.c_id = c_staff.id                                                   "
        + "         LEFT JOIN m_staff u_staff ON t1.u_id = u_staff.id                                                   "
        + "         LEFT JOIN v_dict_info AS t3 ON t3.code = '" + PerfectDictConstant.DICT_SYS_DELETE_MAP + "' and t3.dict_value = cast(t1.is_del as char(1))      "
        + "         LEFT JOIN v_dict_info AS t4 ON t4.code = '" + PerfectDictConstant.DICT_SYS_COMPANY_TYPE + "' and t4.dict_value = t1.type    "
        + "  where true                                                                                                 "
        + "    and (t1.name like CONCAT ('%',#{p1.name,jdbcType=VARCHAR},'%') or #{p1.name,jdbcType=VARCHAR} is null)   "
        + "    and (t1.is_del =#{p1.is_del,jdbcType=VARCHAR} or #{p1.is_del,jdbcType=VARCHAR} is null)                  "
        + "    and (t1.tenant_id =#{p1.tenant_id,jdbcType=BIGINT} or #{p1.tenant_id,jdbcType=BIGINT} is null)           "
    ...
    @Select("<script>                                                                                                   "
        ...
        + "        </foreach>                                                                                           "
        + "  </script>")
    List<MCompanyVo> selectIdsInForExport(@Param("p1") List<MCompanyVo> searchCondition, @Param("p2")Long tenant_id);
