...
//        + "		   					   and t2.dict_value >= t.type                                                   "
    ...
    String groupListSql = "                                                                                              "
        ...
        + "     LEFT JOIN (select *                                                                                      "
        + "               from v_org_route_parent_info                                                                   "
        + "              where type = '" + PerfectDictConstant.DICT_ORG_SETTING_TYPE_GROUP_SERIAL_TYPE + "'              "
        + "            ) vor on t1.id = vor.current_id                                                                    "
        + "         where true                                                                                           "
        + "           and (t1.tenant_id = #{p1.tenant_id,jdbcType=BIGINT} or #{p1.tenant_id,jdbcType=BIGINT} is null)    "
        +"                                                                                                               ";
    /**
    ...
    String companyListSql = "                                                                                           "
        ...
        + "     LEFT JOIN (select *                                                                                      "
        + "               from v_org_route_parent_info                                                                   "
        + "              where type = '" + PerfectDictConstant.DICT_ORG_SETTING_TYPE_COMPANY_SERIAL_TYPE + "'            "
        + "            ) vor on t1.id = vor.current_id                                                                   "
        + "  where true                                                                                                 "
        + "    and (t1.tenant_id = #{p1.tenant_id,jdbcType=BIGINT} or #{p1.tenant_id,jdbcType=BIGINT} is null)          "
        + "                                                                                                             ";
